...
resource "azurerm_resource_group" "rg" {
  name     = "finops-agent-rg"
  location = var.location
}

## Create a shared Log Analytics workspace for monitoring and diagnostics
##
resource "azurerm_log_analytics_workspace" "shared_logs" {
  name                = "log-shared-${random_string.unique.result}"
  location            = var.location
...
## Wait 30 seconds for the AI Foundry account to finish provisioning
##
resource "time_sleep" "wait_ai_foundry_provisioning" {
  depends_on = [
    azurerm_cognitive_deployment.aifoundry_deployment_gpt_4o
  ]
  create_duration = "30s"
}
...
resource "azurerm_private_endpoint" "pe_aifoundry" {
  depends_on = [
    ...
    time_sleep.wait_ai_foundry_provisioning,
    azurerm_virtual_network.vnet
  ]
